     ...
     if ($count > 0) {
        ...
        if ($update_stat == 1) {
            ...
            $stmt = $conn->query("UPDATE Automarket_Invs_web SET 
                                    ...
                                    stat_master = 1, Internacional = '".$Internacional."', tipo_compra = '".$tipo_compra."' WHERE VIN = '".$VIN."'");
        ...
        $stmt = $conn->query("INSERT INTO Automarket_Invs_web 
                                (Year, Transmission, Color, Make, Km, Code, LicensePlate, Model, Chasis, Unit, Engine, Fuel, Price, PriceTax, Doors, CarType, CC, LocationCode, LocationName, Interior, Headline, Description, Photo, Status, Marked, Promo, PromoPrice, PromoPriceTax, LoadDate, Prefijo, VIN, trg_updatefechaWeb, update_stat, stat_master, Internacional, tipo_compra) 
                                VALUES ('".$Year."', '".$Transmission."', '".$Color."', '".$Make."', '".$Km."', '".$Code."', '".$LicensePlate."', '".$Model."', '".$Chasis."', '".$Unit."', '".$Engine."', '".$Fuel."', '".$Price."', '".$PriceTax."', '".$Doors."', '".$CarType."', '".$CC."', '".$LocationCode."', '".$LocationName."', '".$Interior."', '".$Headline."', '".$Description."', '".$Photo."', '".$Status."', '".$Marked."', '".$Promo."', '".$PromoPrice."', '".$PromoPriceTax."', '".$LoadDate."', '".$Prefijo."', '".$VIN."', '".$trg_updatefechaWeb."', '".$update_stat."', 1, '".$Internacional."', '".$tipo_compra."')");

        $stmt = $conn->query("INSERT INTO wp_posts
                                (post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, comment_count)
                                VALUES
                                ('1', NOW(), NOW(), '".$Description."', '".$Model."', '".$Make.' - '.$Model."', 'publish', 'closed', 'closed', '', '".$Model."', '', '', NOW(), NOW(), '', '0', '', '0', 'product', '0')");

        $ultimo_id = $conn->prepare("SELECT MAX(post_id) as post_id FROM wp_posts");
        $ultimo_id->execute();
        $result = $ultimo_id->get_result();
        $row = $result->fetch_array();
        $id_ultimo = $row[0];

        
        $stmt = $conn->query("INSERT INTO wp_postmeta (post_id, meta_key, meta_value) VALUES
                                ('".$id_ultimo."', '_visibility', 'visible'),
                                ('".$id_ultimo."', '_stock_status', 'instock'),
                                ('".$id_ultimo."', '_regular_price', '".$Price."'),
                                ('".$id_ultimo."', '_price', '".$Price."'),
                                ('".$id_ultimo."', '_sale_price', ''),
                                ('".$id_ultimo."', '_purchase_note', ''),
                                ('".$id_ultimo."', '_featured', 'no'),
                                ('".$id_ultimo."', '_weight', ''),
                                ('".$id_ultimo."', '_length', ''),
                                ('".$id_ultimo."', '_width', ''),
                                ('".$id_ultimo."', '_height', ''),
                                ('".$id_ultimo."', '_sku', 'SKU1234'),
                                ('".$id_ultimo."', '_product_attributes', 'a:0:{}'),
                                ('".$id_ultimo."', '_sale_price_dates_from', ''),
                                ('".$id_ultimo."', '_sale_price_dates_to', ''),
                                ('".$id_ultimo."', '_price', '".$Price."'),
                                ('".$id_ultimo."', '_sold_individually', ''),
                                ('".$id_ultimo."', '_manage_stock', 'no'),
                                ('".$id_ultimo."', '_backorders', 'no'),
                                ('".$id_ultimo."', '_stock', '')");        
